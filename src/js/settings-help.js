...
(function () {
  var DESC = [
    ...
    // ---- 工具 ----
    { sel: '#row-export', name: '导出数据', d: '把本机全部数据（所有桌面的聊天 / 字卡 / 设置等）导出为一份备份文件，用于换机或防丢失。\n机制：纯本地导出，无后端。\n影响范围：全量数据。\niOS Safari 可能被系统清空存储，建议定期导出；本行也是「定期备份提醒」记录导出时间的来源。' },
    { sel: '#row-import', name: '导入数据', d: '从备份文件恢复数据，会覆盖 / 合并本机现有数据。\n机制：导入后触发数据回填，请等待恢复完成再操作。\n影响范围：全量数据。\n建议：导入前先「导出数据」留底。' },
    ...
    { sel: '#row-card-audit', name: '字卡使用状态自检', d: '进入字卡自检页：一次看清「自定义字卡」与「系统预设字卡」当前能不能被联系人用到、用不到是卡在哪一步。\n覆盖：二级密码锁状态与影响范围（哪些池被锁停、哪些豁免）、默认聊天字卡/词典/其他互动功能字卡的开关与概率、单卡关闭与分组停用、情绪/回应/心情/情话/位置等字卡池、公用库与各桌面专属库的可用量。\n机制：只读诊断，不改动任何开关或数据；随解锁、切桌面、数据回填实时刷新，可一键复制报告。\n影响范围：当前桌面 + 公用库明细 + 全部桌面的库体积概览。' },
