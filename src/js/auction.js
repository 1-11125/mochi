...
(function () {
  ...
  function addCustomModal() {
    // FIX 2026-09-22 #1033「自制拍品点了没反应」（用户实报，全机型必现）：openModal 的 ctl 只有
    // text() 一个值读写口（全站 31 处同款），此前五处误用不存在的 ctl.val()——stay() 已置「不关窗」
    // 后 val() 抛 TypeError 掐断回调，后续 ph/okText 全不执行＝输入名称点确定弹窗纹丝不动。
