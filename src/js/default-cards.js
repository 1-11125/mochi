...
// 这些字卡不是聊天通用回复，是触发对应功能时联系人才会使用。
  ...
  const DCF_DESC = {
    ...
    care: '【TA的关心（经期）】经期中、经前预警日（提前天数见经期页「提醒设置」）、或推迟 ≥5 天时，联系人主动发消息。v3.42.x #553 起按语境分流：经期中发「经期关心」（附「经期关心」标签，语料 = 字卡库「经期」tab「经期关心」分组 + 经期页自管理关心语）；经前预警日/推迟日发「经期预警」（附「经期预警」标签，语料 =「经前预警」「经期推迟」分组，{d} 自动替换为具体天数）——经期还没到不会再发「经期中」口吻的关心。\n触发时机：TA 每条文字/表情/图片回复后、打开经期页、保存提醒设置时各判定一次；每个语境每天最多一条；无深夜静默。\n本概率 = 在原有闸门之上统一放行（默认 100%）：经期中第 1–2 天另有 90%、第 3–4 天 70%、第 5+ 天 55%，经前预警/推迟预警 75% 的基数；本项 100% = 保持原节奏，0% = 完全不发。\n更彻底：到「经期记录」页把「梦角关心」按钮也关掉（两者都关才真的完全关）。',
