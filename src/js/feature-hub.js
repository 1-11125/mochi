...
(function () {
  ...
  const HUB = [
    ...
    { g: '系统与设置', items: [
      ...
      { n: '通话半框背景图片', d: '给通话半框单独上传背景图（与通话背景互不影响，按联系人各自保存）', k: '通话 半框 背景 图片 上传 壁纸', go: ['#row-call-settings', '#call-half-bg-row'] },
      { n: '移除通话半框背景', d: '清除通话半框背景图恢复默认', k: '通话 半框 背景 移除 删除 清除', go: ['#row-call-settings', '#call-half-bg-remove'] },
      ...
      { n: '通话半框背景（半框内入口）', d: '不用去设置页，在通话半框里直接上传/移除半框专属背景，与「通话半框背景图片」同款', k: '通话 半框 背景 图片 上传 壁纸 移除', go: ['.app[data-app="chat"]', '#more-call', '#call-half-bg-edit-row'] },
      ...
      { n: '屏幕位置微调', d: '顶部/底部/页面高度三轴手动偏移（±80px），不同设备的屏幕错位自己调，本机永久保存', k: '屏幕 微调 位置 偏移 顶部 底部 页面高度 错位 悬空 遮挡 裁切 自己 调', go: ['#row-screen-adj-top'] },
