...
// 桌面补全批（v3.27.x，用户 2026-09-19）：9 组 → 10 组，新增【桌面应用】组（29 个桌面图标的
// 逐页索引 + 一条「图标不见了怎么找回」），并把此前只能开应用、进不到里面各子页的那些入口
// 补齐成两段链（开应用 → 点它页内的那枚按钮），共补 50 条（逐条无头实测落点，见
// tools/verify-fhub-desktop-apps.mjs）。两条口径：①只收录「开页面 / 开面板 / 开弹窗 / 进管理模式」
// 这类入口，**不收会直接改数据的动作按钮**（如经期的「标记今天开始」点了当场记一笔、
...
(function () {
  ...
  const HUB = [
    // v3.27.x 补全批（用户 2026-09-19「功能大全里还缺少桌面的应用啊。根本没有补全这个项目的全部」）：
    // 新增【桌面应用】组＝手机桌面的图标索引。此前 29 个桌面图标虽然各自散落在「聊天传讯/互动与心意/
    // 手机桌面与工具/记录与统计」四个组里（都能搜到），但没有任何一处完整对应「桌面上有哪些应用」，
    // 用户按「桌面应用」找就是空的。本组按桌面第 1/2/3 页的真实排列顺序逐页列出全部 29 个图标，
    // 行名带「（桌面图标）」后缀＝与主题组里的同名条目区分开（同站既有的做法，如「外观与主题（总入口）」），
    // 描述里写明图标在第几页，k 里放「桌面 图标 第N页 找不到 在哪」——装修挪过图标的人也能搜到。
    { g: '桌面应用', items: [
    ...
    { g: '聊天传讯', items: [
      ...
      // #1180（用户直派「回复条数只管基础回复，撤回补发/逐卡连发/心情分享/红包捎话/主动发送全都绕过上限，聊天里联系人发送的消息非常多」）：
      // 限流功能本身＋「为什么比设的还多」这条解释都要能被搜到
      { n: 'TA 消息限流（总量限流）', d: '默认关闭。打开后任意一段「限流窗口」时间里，TA 最多发「窗口内最多条数」条消息，超出的直接不再发出、窗口过去自动恢复；回复、逐卡连发、撤回补发、心情分享、红包/心意币/听歌邀请、主动发送全部一起计数一起拦（只有已读回执与你当刻操作引发的记录不占额度）。在 回复设置 → 聊天 →「总量限流」组，按联系人独立生效', k: '限流 总量 刷屏 消息太多 太多 烦 条数上限 安静 压制 防刷屏 窗口 分钟 回复设置', go: ['#row-general'] },
      { n: '为什么 TA 发的比「回复条数」还多', d: '「回复条数最少/最多」＝你每发 1 条消息 TA 跟着回你几条，不是 TA 一天最多发几条。不占它名额、会额外多发的有：词典拼字的多回复逐卡连发（默认不受条数限制）、撤回后补发、TA 的心情分享、红包/心意币/听歌邀请、TA 主动发送（连发几条由「主动发送→发送条数」单独控制）。想管住总量就打开同面板下方的「TA 消息限流」', k: '回复条数 为什么 多 刷屏 消息太多 撤回补发 逐卡连发 心情分享 红包 听歌 邀请 主动发送 名额 上限 限流', go: ['#row-general'] },
    ...
    { g: '互动与心意', items: [
      ...
      // #848：送礼后 TA 的回应、红包被领后的捎话，两处都要能被搜到（用户实报「设置里没看到这个功能」）
      ...
      // ---- 桌面补全批：以下为「开了应用之后进不到里面」的那些入口，两段链＝开应用 → 点它页内那枚按钮 ----
    ...
    { g: '手机桌面与工具', items: [
      ...
      // ---- 桌面补全批：桌面小应用各自的页内入口（原先只能开应用，进不到它的设置/面板）----
    ...
    { g: '记录与统计', items: [
      ...
      // 桌面补全批修正：本行原先只 go 到日历页，停在日历首页、进不了心情日记本身
      //（用户搜「心情日记」点进去看不到日记）——补上日历里的「心情日记」入口那一步。
  ...
  // 顺序与 HUB 分组一一对应：新增分组必须同步在对应位置插一枚图标（插在头部即整组顺移，不会串位）。
  const CAT_ICO = [
    // 桌面应用（3×3 应用抽屉点阵）
