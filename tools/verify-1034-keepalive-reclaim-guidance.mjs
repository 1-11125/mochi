...
A('S1 诊断回收警告带「止住它最有效」动作（源+产物）', has(devSrc, '止住它最有效：Chrome 设置→性能→「内存节省程序」关掉') && has(idx, '止住它最有效：Chrome 设置→性能→「内存节省程序」关掉'));
A('S2 诊断回收警告带恢复口径（自动重载/碰一下接上/数据不丢）', has(devSrc, '保活在你碰一下页面时自动接上，数据不会丢'));
A('S3 回收提示条带动作与恢复口径（产物 js/bg-keep.js）', has(bk, '止住它最有效的一步：Chrome 设置→性能→「内存节省程序」关掉') && has(bk, '保活在你碰一下页面时自动接上'));
