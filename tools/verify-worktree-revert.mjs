...
// 判据（零机型／零环境猜测）：某条哨兵的 needle 在 `HEAD:src/<file>` 里在、在工作树
// `src/<file>` 里不在 ⇒ 这次构建会打回它。`absent` 型（禁止回流的写法）判据反向。
...
//       退出码 0＝无打回 / 1＝有打回（build.mjs 据此拒绝写产物，`--allow-revert` 可放行）/ 2＝跳过
...
function loadRegistry() {
  ...
  if (start < 0) return { err: 'HEAD 的 build.mjs 里没有 FIX_SENTINELS 数组' };
  ...
  return { list: list, head: safeHead() };
}
...
const headCache = {}, wtCache = {};// 只认 blob：实测有 `file: 'js/'` 这种「全族禁回流」的目录形态登记（#756z）。对目录 `git show`
...
export function checkReverts() {
  ...
  return { head: reg.head, total: reg.list.length, comparable: revert.length + onlyWt.length + sameOk + sameBad,
    revert: revert, onlyWt: onlyWt, sameOk: sameOk, sameBad: sameBad, skipped: skipped };
}
...
if (process.argv[1] && normalize(process.argv[1]).replace(/\\/g, '/').indexOf('verify-worktree-revert') >= 0) {
  ...
  if (!r.revert.length) {
    console.log('✅ [打回体检] 工作树相对 HEAD(' + r.head + ') 无缺失修复锚点——可比对 ' + r.comparable + '/' + r.total + ' 条，两侧同无 ' + r.sameBad + ' 条（存量哑针，非本批引入）');
    process.exit(0);
  }
  const byBatch = {};
  ...
  console.error('   现在构建＝上面这些修复从 index.html / js/*.js 里消失（「修了没上线」）。处置：先在最新 tip 上重放本批改动，' +
    '确认无碍再 `node build.mjs --allow-revert` 放行。');
  process.exit(1);
}
