...
// #588 修用户报障「提示条挡住使用」（多机型）：提示条原为绝对定位压在照片顶部（照片高约 104px 时占 33px
...
//   E1/E2 照片**顶部那一条**不再被提示条占住，点下去照常落位（#588 主诉，旧实现必红）；
...
// A2（#588 契约）：提示条不得落在配图区内＝不得覆盖照片——压在上面就是「点顶部被提示条接走」的复发形态
